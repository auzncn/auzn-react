...
export default function ExerciseCard(props) {
    ...
    const [setsComplated, setSetsComplated] = useState(0);

    function handleSetIncrement() {
        setSetsComplated((setsComplated + 1) % 6);
    }
    return (
        <div className=' p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap'>
            <div className='flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4 '>
                ...
                <p className='text-sm test-slate-400 capitalize'>{exercise.type}</p>
            </div>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2'>
                ...
                <button className='flex flex-col p-2 rounded border-[1.5px] duration-200 border-solid border-blue-900 hover:border-blue-600 w-full'
                    ...
                    <h3 className='text-slate-400 text-sm capitalize'>Sets complated</h3>
                    <p className='font-medium'>{setsComplated} / 5</p>
                </button>
            </div>
        </div>
    )
